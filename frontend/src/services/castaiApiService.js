...
export const fetchProblematicNodes = async (clusterId, region, apiKey) => {
    ...
    const response = await fetch(
        `https://ceb.tech-sphere.pro/clusters/${clusterId}/problematic-nodes?region=${region}&api_key=${apiKey}`,
        {
            headers: {
...
export const fetchProblematicWorkloads = async (clusterId, region, apiKey, aggressiveMode = false) => {
    ...
    const response = await fetch(
        `https://ceb.tech-sphere.pro/clusters/${clusterId}/problematic-workloads?region=${region}&api_key=${apiKey}&aggressive_mode=${aggressiveMode}`,
        {
            headers: {
                'accept': 'application/json'
            }
